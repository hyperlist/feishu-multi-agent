...
3. **处理飞书 API 限制**：
   ...
   - 或手动在飞书开放平台后台添加用户
4. **更新配置**：添加 agent、binding、群聊配置到 openclaw.json
5. **更新文档**：
   - main 的 TOOLS.md（agent 列表 + 权限矩阵）
   - 新 agent 的 workspace 文档（SOUL.md/AGENTS.md/USER.md/TOOLS.md）
6. **分享技能**：复制 delegate-agent 到新 agent 的 skills 目录
7. **重启 gateway**：配置生效
8. **验证**：在群聊中测试，确认 agent 正常工作

### 常见问题与解决方案

| 问题 | 原因 | 解决方案 |
|------|------|----------|
| 232024 错误：用户不在应用可见范围 | 用户没有授权或不在应用成员列表 | 1. 让用户先给机器人发私信<br>2. 在飞书开放平台后台添加用户<br>3. 让管理员添加 |
| 创建群聊后无法拉人 | 用户跨企业或不在同一租户 | 检查用户是否在同一飞书企业内 |
| Agent 创建后无法通信 | sessionKey 格式错误 | 格式：`agent:<id>:feishu:group:oc_xxx` |
| 权限配置不生效 | gateway 未重启 | `openclaw gateway restart` |
